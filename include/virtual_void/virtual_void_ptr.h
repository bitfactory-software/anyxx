...
private:
    ...
    unique_void( std::unique_ptr< void, deleter_t > ptr, v_table* v_table )
...
{
    unique_void::deleter_t deleter = +[]( void* p ){ delete static_cast< T* >( p ); };
    return unique_void{ std::unique_ptr< void, unique_void::deleter_t >( new T( std::forward< ARGS >( args )... ), deleter ), v_table_of< T >() };
}

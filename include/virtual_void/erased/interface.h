  ...
  struct n : BASE<LIFETIME_HOLDER> {                                           \
   public:                                                                     \
    ...
    using query_interface_unique_t =                                           \
        n##interface<virtual_void::erased::base<lifetime_holder_t>>;           \
    template <typename T>                                                      \
    using is_already_base =                                                    \
...
#define ERASED_FREE_INTERFACE(name, ...)                     \
  _detail_ERASED_INTERFACE(name, virtual_void::erased::base, \
                           _detail_INTERFACE_FREE_LIMP_H, (__VA_ARGS__))
...
template <is_erased_lifetime_holder LIFETIME_HOLDER,
          template <typename> typename BASE, typename RET, typename... ARGS>
struct call_operator_facade;
template <is_erased_lifetime_holder LIFETIME_HOLDER,
          template <typename> typename BASE, typename RET, typename... ARGS>
struct call_operator_facade<LIFETIME_HOLDER, BASE, RET(ARGS...)>
    : BASE<LIFETIME_HOLDER> {
 public:
  using lifetime_holder_t = LIFETIME_HOLDER;
  using void_t = LIFETIME_HOLDER::void_t;
...
//     base > using apply = call_operator_facade< LIFETIME_HOLDER, BASE, RET,
//     ARGS... >;
// };

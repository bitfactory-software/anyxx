...
class shared_const {
 protected:
  ...
  explicit shared_const(const shared_abstract_data_ptr& ptr) : ptr_(ptr) {}
 ...
 public:
  ...
  shared_const(std::in_place_type_t<T>, ARGS&&... args) noexcept
      : ptr_(std::make_shared<concrete_data<T> >(
  ...
  template <typename U>
  friend U* reconcrete_cast(shared_const&);
  template <typename U>
  friend const U* reconcrete_cast(const shared_const&);
};
template <typename U>
U const* reconcrete_cast(const shared_const& u) {
...
class typed_shared_const : public shared_const {
 ...
 public:
  ...
  typed_shared_const(T&& v) noexcept : shared_const(std::forward<T>(v)) {}
  typed_shared_const(const T& v) noexcept : shared_const(std::forward<T>(v)) {}

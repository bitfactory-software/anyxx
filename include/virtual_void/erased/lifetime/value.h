...
using abstract_value_data_clone_f = value_abstract_data_ptr(*)(abstract_value_data const*);
struct abstract_value_data_cloneable 
{
	abstract_value_data_clone_f clone_;
};
template< typename M > struct concrete_value_data : concrete_data_t< M, abstract_value_data_cloneable >
{
	static value_abstract_data_ptr clone_impl( abstract_value_data const * from_abstract ) 
	{ 
	...
	concrete_value_data( std::in_place_t in_place,  ARGS&&... args )
		: concrete_data_t< M, abstract_value_data_cloneable >( in_place, std::forward< ARGS >( args )... )
	{
		this->clone_ = &clone_impl;
	}
};
...
protected:
	explicit value( value_abstract_data_ptr&& ptr )
		: ptr_( std::move( ptr ) )
	{}
public:
	using void_t = void *;
	...
	value( std::in_place_type_t< T >, ARGS&&... args ) noexcept
		: ptr_( std::make_unique< concrete_value_data< T > >( std::in_place, std::forward< ARGS >( args )... ) )
	{}
	void* data() const { return ptr_->data_; }
	operator bool() const { return ptr_.operator bool(); } // false only after move!
	template< typename U > friend U* reconcrete_cast( value& );
	template< typename U > friend const U* reconcrete_cast( const value& );
};
template< typename U > U* reconcrete_cast( value& u ) 
{ 
	return reconcrete_cast< U, abstract_value_data_cloneable >( *u.ptr_ ); 
}
template< typename U > const U* reconcrete_cast( const value& u ) 
{ 
	return reconcrete_cast< U, abstract_value_data_cloneable >( *u.ptr_ ); 
}
...
{
    template< typename T > friend auto as( value&& source );
	typed_value( value&& u ) noexcept
		: value( std::move( u ) ) 
	{}
public:
	using conrete_t = T;
	using value::value;
	typed_value( T&& v ) noexcept
		: value( std::move( v ) )
	{}
	template< typename... ARGS > 
...
template< typename T > auto as( value&& source )
{
    return typed_value< T >{ std::move( source ) };
}
template< typename T, typename... ARGS > typed_value< T > make_value( ARGS&&... args )
{

...
//-----------------------------------------------------------------------------
// evaluate
extension_method<node::model::v_table_t, int(virtual_void::const_)> value;
...
//-----------------------------------------------------------------------------
// render as Forth
extension_method<node::model::v_table_t, std::string(virtual_void::const_)>
    as_forth;
...
////   return "(times " + as_lisp(expr->left) + " " + as_lisp(expr->right) +
///")"; / }); / auto __ = dump_as_lisp.center.define<Integer>( /     [](Integer
/// const* expr, auto out, auto) /     { /       auto s =
/// unerase_cast<std::string*>(out); /       (*s) +=

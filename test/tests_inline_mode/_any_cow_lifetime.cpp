...
TEST_CASE("cow lifetime") {
  {
    CHECK(X::tracker_ == 0);
    {
      any<getset> u{std::in_place_type<X>, "hallo"};
      CHECK(u.get() == "hallo");
      CHECK(X::tracker_ == 1);
    }
    CHECK(X::tracker_ == 0);
  }
  {
    CHECK(X::tracker_ == 0);
    {
      ...
      CHECK(u.get() == "hallo");
      CHECK(X::tracker_ == 1);
      any<getset> u2{std::in_place_type<X>, "world"};
      CHECK(u2.get() == "world");
      CHECK(X::tracker_ == 2);
      u2 = std::move(u);
      ...
      CHECK(u3.get() == "hallo");
      CHECK(X::tracker_ == 1);
      auto u4 = move_to<any<getset>>(std::move(u3));
  ...
  }
  {
    CHECK(X::tracker_ == 0);
    {
      any<getset> v{std::in_place_type<X>, "hallo"};
      CHECK(v.get() == "hallo");
      CHECK(X::tracker_ == 1);
      auto v2 = v;
      CHECK(X::tracker_ == 2);

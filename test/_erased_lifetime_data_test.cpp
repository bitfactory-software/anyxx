...
#define DATA_ALIGNED(T, META_DATA) data::typed<T, META_DATA>
...
#define TRACE_OFFSET_EMPTY(T)                                     \
  {                                                               \
    using TYPE = data::typed<T, data::has_no_meta>;               \
    std::cout << "data::typed<" << #T                             \
...
#define TRACE_OFFSET(T, META_DATA)                                \
  {                                                               \
    using TYPE = data::typed<T, META_DATA>;                       \
...
TEST_CASE("erase lifetiem test unique") {
  ...
  {
    auto unique_ptr = erased::data::make_unique<data::typed<Data,data::has_no_meta>>();
...
TEST_CASE("erase lifetiem test shared") {
  Data::destrucor_runs = 0;
  {
    std::shared_ptr<data::with_no_meta const> sp =
        data::make_shared_const<data::typed<Data, data::has_no_meta>>();

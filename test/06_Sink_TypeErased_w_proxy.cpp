...
struct EntityFacade
    : pro::facade_builder ::add_facade<virtual_void::proxy::virtual_void_facade,
                                       true>
      //        ::add_facade< virtual_void::proxy::m_table_facade, true >
      ::add_convention<FreeToString, std::string() const>::support_copy<
...
struct System {
  ...
  void Query(std::string what, const SinkFunction& sink) {
    auto dataLines =
        std::vector<std::pair<std::string, std::string> >{{"i", "1"},
                                                          {"i", "4711"},
                                                          {"s", "hello"},
                                                          {"s", "world"},
                                                          {"d", "3.14"}};
...
auto entityToOut =
    virtual_void::typeid_::open_method<void(const void*)>{applicationDomain};
...
TEST_CASE("06_Sink_TypeErased_w_proxy") {
  ...
  db.factories["i"] = [](const std::string& data) -> DB::Entity {
    return std::make_shared<IntData>(std::atoi(data.c_str()));
  };
  db.factories["s"] = [](const std::string& data) -> DB::Entity {
    return std::make_shared<StringData>(data);
  };
  db.factories["d"] = [](const std::string& data) -> DB::Entity {
    return std::make_shared<DoubleData>(std::atof(data.c_str()));
  };
  db.Query("junk", [&](const DB::Entity& e) {
    try {
      ...
      std::cout
          << "type_info: "
          << pro::proxy_reflect<virtual_void::proxy::meta>(e).type_info.name()
          << ": " << ToString(*e) << std::endl;

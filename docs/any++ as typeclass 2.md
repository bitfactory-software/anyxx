...
it is inherently self referential, it shows the obstacles an implementation of type has to overcome.

Let's start with a colloquial definition of the Monoid:
...
The Haskell definition of the Monoid type class looks like this:
```haskell
...
``mappend`` requires the binary operation.
``mempty`` and ``mconcat`` have default implementations, which reference each other. ``mconcat`` is defined in terms of ``mappend`` and ``mempty``, and ``mempty`` is defined as the result of concatenating an empty list.
...
This is a powerful feature of type classes, because it allows to define a lot of functionality for a type class, where a concrete type only has to provide a few implementations to get all the other functionality for free.
...
We can see that the Monoid trait with its three requirements, which are the same as the Haskell type class.
...
How far can we get there with standard C++23?

Gracefully to Steve Downey, we can get pretty far, let's see his approach to \ref subpage3 "C++0x Concepts with Concept Maps using C++23 Library Tech" in the next chapter.

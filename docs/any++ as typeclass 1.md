...
It meant a way to define "concepts", which are sets of requirements that a type must satisfy.
With this feature, errors would be detected at the point of template instantiation, if the caller made a mistake, or inside the template, if the implementer used the parameters wrong.
The proposal included concept maps, which are a way to define how a type fits a concept, and to provide default implementations for the requirements of a concept.
...
and are far more powerful than C++20 Concepts Lite. 
...
C++ has some advanced Type Erasure libraries, but the compile time aspect of type classes is unaddressed.
...
Any++ offers a complete solutions for the last points, and a partial solution for the first. 

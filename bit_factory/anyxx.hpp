...
#define _detail_ANYXX_DUMMY_INT_PARAM_LIST(...) \
  _detail_EXPAND_(_detail_ANYXX_DUMMY_INT_PARAM_LIST_H(__VA_ARGS__))
...
#define _detail_ANYXX_CONCEPT_TYPE(template_params, name, erased, default_) \
  !std::same_as<typename decltype(model_map)::template name<                \
                    _detail_ANYXX_DUMMY_INT_PARAM_LIST_H(                   \
                        a, _sig, ANYXX_UNPAREN(template_params))>,          \
                anyxx::undefined>;
...
#define _detail_ANYXX_TRAIT_ERROR_MESSAGE(name, ret)                          \
  ...
  struct is_type_complete_impl<
      ...
      : std::true_type {};
  template <typename T>
  constexpr static inline bool is_type_complete =
      is_type_complete_impl<T>::value;
  ...
  inline mutable_void copy_construct_at(
      any_v_table * v_table, mutable_void placement, const_void from) {
  ...
  inline mutable_void move_construct_at(
      any_v_table * v_table, mutable_void placement, mutable_void from) {
  ...
  auto visit_value(auto&& visitor, val& v1, std::size_t size1, V2&& v2,
    ...
    if (size2 > sizeof(mutable_void)) {
      return std::forward<decltype(visitor)>(visitor)(
          v1.trivial, std::forward<V2>(v2).heap);
    } else if (size2 > 0) {
      return std::forward<decltype(visitor)>(visitor)(
          v1.trivial, std::forward<V2>(v2).local);
    }
    assert(size2 == 0);
    return std::forward<decltype(visitor)>(visitor)(
        v1.trivial, std::forward<V2>(v2).trivial);
  };
   ...
   public:
    ...
    std::expected<any_v_table*, cast_error> get_v_table(
      ...
      return std::unexpected(
          cast_error{.to = typeid_, .from = get_type_info()});
    }
  ...
  class any : public v_table_holder<is_dyn<Proxy>, Trait>, public Trait {
   ...
   public:
    ...
    template <is_any Other>
    explicit(false)
        any(Other&& other) noexcept  // NOLINT(noExplicitConstructor)
  ...
  using v_table_return =
      TRAIT_TYPE(v_table_return, Return, translate_sig, AnyValue);
  ...
  struct translate_erased_function {
    using type =
        RET (*)(typename translate_erased_function_param<Args>::type...);
  };
    ...
    struct dispatch_access<kind::multiple, dimension_count, DispatchArgs...> {
      ...
      std::optional<R> invoke(F const& target, ArgsTuple&& dispatch_args_tuple,
        ...
        auto typed_target = reinterpret_cast<
            typename dispatch_function<R, std::decay_t<ArgsTuple>>::type>(
            target);
      ...
      std::optional<R> invoke(DispatchMatrix const& target,
        ...
        if (target.size())
          return next_t::invoke(
              target[0],
              std::tuple_cat(
                  get_tuple_head<Dimension>(dispatch_args_tuple),
                  std::make_tuple(&any),
   ...
   public:
    ...
    auto operator()(ActualArgs&&... actual_args) const {
      if constexpr (dispatch_kind == kind::multiple) {
        ...
        return dispatch_access_.invoke(
            dispatch_default_hook_, std::forward<ActualArgs>(actual_args)...);
      }
    }
    ...
    auto invoke1(Vany1&& vany, Args&&... args) const {
      return std::visit(
          [&]<typename TypedArg>([[maybe_unused]] TypedArg&& arg) {
            return overloads{
                StaticDispatch, [&]<is_any Any, typename... Vargs>(
                                    Any&& any, Vargs&&... vargs) {

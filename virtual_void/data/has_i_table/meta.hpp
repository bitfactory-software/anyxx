...
concept is_i_table_meta = is_meta<META> && requires(META meta) {
  {
    meta.get_i_table()
  } -> std::convertible_to<interface::i_table*>;
};
...
struct meta {
  ...
  constexpr type_info_ptr type_info() const { return &get_i_table()->type(); }

  constexpr interface::i_table* get_i_table() const {
    return i_table_;
  }

...
static_assert(std::sentinel_for<dyn<rangesxx::sentinel, int>, dyn<rangesxx::forward_iterator, int>>);
static_assert(std::sentinel_for<dyn<rangesxx::sentinel, int>, dyn<rangesxx::bidirectional_iterator, int>>);
static_assert(std::sentinel_for<dyn<rangesxx::sentinel, int>, dyn<rangesxx::random_access_iterator, int>>);
static_assert(std::sentinel_for<dyn<rangesxx::sentinel, int>, dyn<rangesxx::contiguous_iterator, int>>);
...
void test_forward_range(dyn<rangesxx::view, rangesxx::forward, int const> r) {
    auto expected = 1;
    std::ranges::for_each(r, [&expected](int x){ CHECK(x == expected++); });
}
